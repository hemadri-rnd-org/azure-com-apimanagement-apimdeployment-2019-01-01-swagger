...
func Apimanagementservice_restoreHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ApiManagementServiceBackupRestoreParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/restore%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateApimanagementservice_restoreTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_restore",
		mcp.WithDescription("Restores a backup of an API Management service created using the ApiManagementService_Backup operation on the current service. This is a long running operation and could take several minutes to complete."),
		...
		mcp.WithString("backupName", mcp.Required(), mcp.Description("Input parameter: The name of the backup file to create.")),
		mcp.WithString("containerName", mcp.Required(), mcp.Description("Input parameter: Azure Cloud Storage blob container name used to place/retrieve the backup.")),
		mcp.WithString("storageAccount", mcp.Required(), mcp.Description("Input parameter: Azure Cloud Storage account (used to place/retrieve the backup) name.")),
		mcp.WithString("accessKey", mcp.Required(), mcp.Description("Input parameter: Azure Cloud Storage account (used to place/retrieve the backup) access key.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apimanagementservice_restoreHandler(cfg),
	}
}
