...
func Apimanagementservice_getssotokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/getssotoken%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
		...
		var result models.ApiManagementServiceGetSsoTokenResult
...
func CreateApimanagementservice_getssotokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_getssotoken",
		mcp.WithDescription("Gets the Single-Sign-On token for the API Management Service which is valid for 5 Minutes."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apimanagementservice_getssotokenHandler(cfg),
	}
}
