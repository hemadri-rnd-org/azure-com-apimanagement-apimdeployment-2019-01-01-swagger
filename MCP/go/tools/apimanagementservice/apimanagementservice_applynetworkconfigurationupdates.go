...
func Apimanagementservice_applynetworkconfigurationupdatesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ApiManagementServiceApplyNetworkConfigurationParameters
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/applynetworkconfigurationupdates%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateApimanagementservice_applynetworkconfigurationupdatesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_applynetworkconfigurationupdates",
		mcp.WithDescription("Updates the Microsoft.ApiManagement resource running in the Virtual network to pick the updated network settings."),
		...
		mcp.WithString("location", mcp.Description("Input parameter: Location of the Api Management service to update for a multi-region service. For a service deployed in a single region, this parameter is not required.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apimanagementservice_applynetworkconfigurationupdatesHandler(cfg),
	}
}
